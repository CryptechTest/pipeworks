...
local function set_filter_formspec(data, meta)
    ...
    if data.digiline then
        formspec = "size[8.5,3]" .. "item_image[0.2,0;1,1;pipeworks:" .. data.name .. "]" .. "label[1.2,0.2;" ..
                       minetest.formspec_escape(itemname) .. "]" .. "field[0.5,1.6;4.6,1;channel;" .. S("Channel") ..
                       ";${channel}]" .. "button[4.8,1.3;1.5,1;set_channel;" .. S("Set") .. "]" ..
                       fs_helpers.cycling_button(meta, "button[0.2,2.3;4.05,1", "slotseq_mode",
                {S("Sequence slots by Priority"), S("Sequence slots Randomly"), S("Sequence slots by Rotation")}) ..
                       fs_helpers.cycling_button(meta, "button[4.25,2.3;4.05,1", "exmatch_mode",
                {S("Exact match - off"), S("Exact match - on")}) .. "button_exit[6.3,1.3;2,1;close;" .. S("Close") ..
                       "]"
    else
        local exmatch_button = ""
        if data.stackwise then
            exmatch_button = fs_helpers.cycling_button(meta, "button[" .. (10.2 - (0.22) - 4) .. ",4.5;4,1",
                "exmatch_mode", {S("Exact match - off"), S("Exact match - on")})
        end
        local size = "10.2,11"
        ...
        if minetest.get_modpath("i3") then
            ...
            for i = 0, 7 do
                for j = 0, 1 do
                    list_backgrounds = list_backgrounds .. "box[" .. 0.22 + (i * 1.25) .. "," .. 1.75 + (j * 1.25) ..
                                           ";1,1;]"
                end
            end
        end
        formspec = "formspec_version[2]" .. "size[" .. size .. "]" .. pipeworks.fs_helpers.get_prepends(size) ..
                       "item_image[0.22,0.22;1,1;pipeworks:" .. data.name .. "]" .. "label[1.22,0.72;" ..
                       minetest.formspec_escape(itemname) .. "]" .. "label[0.22,1.5;" .. S("Prefer item types:") .. "]" ..
                       list_backgrounds .. "list[context;main;0.22,1.75;8,2;]" ..
                       fs_helpers.cycling_button(meta, "button[0.22,4.5;4,1", "slotseq_mode",
                {S("Sequence slots by Priority"), S("Sequence slots Randomly"), S("Sequence slots by Rotation")}) ..
                       exmatch_button .. pipeworks.fs_helpers.get_inv(6) .. "listring[]"
    end
...
local function punch_filter(data, filtpos, filtnode, msg)
    ...
    if not todef or
        not (minetest.get_item_group(tonode.name, "tube") == 1 or minetest.get_item_group(tonode.name, "tubedevice") ==
            1 or minetest.get_item_group(tonode.name, "tubedevice_receiver") == 1) then
        return
    end

    if fromtube then
        fromtube.input_inventory = input_special_cases[fromnode.name] or fromtube.input_inventory
    end
    ...
    if fromtube.before_filter then
        fromtube.before_filter(frompos)
    end
    ...
    local function grabAndFire(frominvname, filterfor)
        ...
        for spos, stack in ipairs(frominv:get_list(frominvname)) do
            ...
            if filterfor == "" then
                ...
                stack:get_name(), fgroup) ~= 0)) and (not fwear -- If there's a wear filter:
        ...
        for _, spos in ipairs(sposes) do
            ...
            if doRemove > 0 then
                ...
                pipeworks.tube_inject_item(pos, start_pos, dir, item, fakePlayer:get_player_name())
    ...
    for _, frominvname in ipairs(type(fromtube.input_inventory) == "table" and fromtube.input_inventory or
                                     {fromtube.input_inventory}) do
        local done = false
    ...
    if fromtube.after_filter then
        fromtube.after_filter(frompos)
    end
end
...
for _, data in ipairs({{
    ...
    local node = {
        ...
        tiles = {"pipeworks_" .. data.name .. "_top.png", "pipeworks_" .. data.name .. "_top.png",
                 "pipeworks_" .. data.name .. "_output.png", "pipeworks_" .. data.name .. "_input.png",
                 "pipeworks_" .. data.name .. "_side.png", "pipeworks_" .. data.name .. "_top.png"},
        paramtype2 = "facedir",
        groups = {
            snappy = 2,
            choppy = 2,
...
local function put_to_inputinv(pos, node, filtmeta, list)
    ...
    local listname = type(fromtube.input_inventory) == "table" and fromtube.input_inventory[1] or
                         fromtube.input_inventory
