...
if pipeworks.enable_mese_tube then
	...
	pipeworks.register_tube("pipeworks:mese_tube2", {
			description = S("Filtering Pneumatic Tube Segment"),
			inventory_image = "pipeworks_mese_tube_inv_b.png",
			noctr = {"pipeworks_mese_tube_noctr_1_b.png", "pipeworks_mese_tube_noctr_2_b.png", "pipeworks_mese_tube_noctr_3_b.png",
				"pipeworks_mese_tube_noctr_4_b.png", "pipeworks_mese_tube_noctr_5_b.png", "pipeworks_mese_tube_noctr_6_b.png"},
			...
			node_def = {
								...
								tube_def.can_insert(npos, node, stack, vect) then
									...
									for _, st in ipairs(inv:get_list(invname)) do
										if not st:is_empty() then
											is_empty = false
											local sname = st:get_name();
											local pattern = ".*"..string.match(sname, "([^:]+)$")..".*"
											if string.match(string.match(name, "([^:]+)$"), pattern) then
												foundn = foundn + 1
												found[foundn] = vect
											end
										end
				...
				on_construct = function(pos)
					...
					update_formspec(pos)
					meta:set_string("infotext", S("Filtering pneumatic tube"))
				end,
